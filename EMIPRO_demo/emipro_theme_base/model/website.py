...
class Website(models.Model):
    ...
    def _get_default_header_content(self):
        ...
        html = """<p></p><div class="s_rating row te_s_header_offer_text"><ul>{}</ul></div>""".format(list_html)
        return html
    ...
    def _get_default_footer_extra_links(self):
        ...
        html = """<section><div class="te_footer_inline_menu"><ul class="te_footer_inline_menu_t">{}</ul></div></section>""".format(list_html)
        return html
    ...
    load_more_image = fields.Binary('Load More Image', help="Display this image while load more applies.",
                                    readonly=False)
    button_or_scroll = fields.Selection([('automatic', 'Automatic- on page scroll'),
                                         ('button', 'Button- on click button')], string="Loading type for products",
                                        required=True, default='automatic', readonly=False)
    prev_button_label = fields.Char(string='Label for the Prev Button', readonly=False, default="Load prev",
                                    translate=True)
    next_button_label = fields.Char(string='Label for the Next Button', readonly=False, default="Load next",
                                    translate=True)
    is_lazy_load = fields.Boolean(string='Lazyload', help="Lazy load will be enabled", readonly=False)
    lazy_load_image = fields.Binary('Lazyload Image', help="Display this image while lazy load applies.", readonly=False)
    ...
    price_filter_on = fields.Selection([('list_price', 'On Product Sale Price'),
                                       ...
                                       string="Price Range Filter For Products", default='list_price', readonly=False)
    ...
    def category_check(self):
        ...
        return self.env['product.public.category'].sudo().search([('parent_id', '=', False),
    ...
    def get_min_max_prices(self, search=False, category=False, attributes=False):
        ...
        if products:
            ...
            cust_min_val = request.httprequest.values.get('min_price', 0.0)
            cust_max_val = request.httprequest.values.get('max_price', 0.0)
            ...
            range_list = []
            minimum_price = round(products[0].price if is_web_price else min_price, 2)
            maximum_price = round(products[-1].price if is_web_price else max_price, 2)
            if not cust_min_val and not cust_max_val:
                range_list = range_list + [minimum_price, maximum_price, minimum_price, maximum_price]
            else:
                range_list = range_list + [round(cust_min_val, 2), round(cust_max_val, 2), minimum_price, maximum_price]
            return range_list
        else:
            return False
    ...
    def list_providers_ept(self):
        ...
        for provider in providers:
            ...
            params = dict(response_type='token', client_id=provider['client_id'], redirect_uri=return_url,
                          scope=provider['scope'], state=json.dumps(state))
    ...
    def get_product_data(self, products, product_count=1):
        product_items = [products[i:i + product_count] for i in range(product_count, len(products), product_count)]
        product_items.insert(0, products[:product_count])
