...
class SynonymGroup(models.Model):
    ...
    name = fields.Char(string='Synonyms Group', required=True,
                       help='Add comma separated keywords, which are synonyms of each other '
                            'to make use of them while searching on website')
    ...
    def check_synonyms_validation(self, synonyms=''):
        ...
        grps = list(map(str.strip,list(chain.from_iterable([g.get('name').split(',') for g in groups]))))
