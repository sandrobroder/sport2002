...
class SaleOrderLine(models.Model):
    ...
    def _compute_agent_ids(self):
        ...
        for record in self:
            if record.order_id.partner_id and not record.commission_free and record.product_id:
                record.agent_ids = record._prepare_agents_vals_partner(
                    record.order_id.partner_id, settlement_type="sale_invoice",
                    product_categ_id=record.product_id.categ_id
                )
    ...
    #     for record in self:
    #         record.commission_id = record.product_id.categ_id.agent_ids
