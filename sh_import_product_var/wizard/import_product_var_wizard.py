                    ...
                    for row in values:
                        ...
                        try:
                            if skip_header:
                                ...
                                print("row_field_dic", row_field_dic)
                                counter = counter + 1
                                continue
                            ...
                            if row[0] not in (None, "") and row[1] not in (None, ""):
                                ...
                                if row[0] != running_tmpl:
                                    ...
                                    print("running_tmpl_domain", running_tmpl_domain)
                                    # =====================================
                                    # Search or Update product by
                                    # =====================================
                                    ...
                                    if row[5].strip() in (None, "") or row[6].strip() in (None, ""):
                                        has_variant = False
                                    ...
                                    if row[12].strip() == 'FALSE':
                                        tmpl_vals.update({'sale_ok': False})

                                    tmpl_vals.update({'purchase_ok': True})
                                    if row[13].strip() == 'FALSE':
                                        tmpl_vals.update(
                                            {'purchase_ok': False})

                                    # Product Type
                                    if row[14].strip() == 'Service':
                                        tmpl_vals.update({'detailed_type': 'service'})
                                    elif row[14].strip() == 'Storable Product':
                                        tmpl_vals.update({'detailed_type': 'product'})
                                    elif row[14].strip() == 'Consumable':
                                        tmpl_vals.update({'detailed_type': 'consu'})

                                    # UOM
                                    if row[15].strip() in (None, ""):
                                        search_uom = self.env[
                                            'uom.uom'].search(
                                            [('name', '=', 'Units')],
                                            limit=1)
                                    ...
                                    if row[17].strip() not in (None, ""):
                                        for x in row[17].split(','):
                                            x = x.strip()
                                            if x != '':
                                    ...
                                    tmpl_vals.update({'is_published': True})
                                    if row[20].strip() == 'FALSE':
                                        tmpl_vals.update({'is_published': False})
                                    ...
                                    if has_variant == False and row[10] not in (None, ""):
                                        if created_product_tmpl and created_product_tmpl.product_variant_id and created_product_tmpl.type == 'product':
                                            ...
                                            created_qty_on_hand = self.env[
                                                'stock.change.product.qty'].create(stock_vals)
                                ...
                                if created_product_tmpl and has_variant:
                                    ...
                                    pro_attr_obj = self.env[
                                        'product.attribute']
                                    if row[5].strip() not in (None, "") and row[6].strip() not in (None, ""):
                                        attr_ids_list = []
                                        for attr in row[5].split(','):
                                            attr = attr.strip()
                                            if attr != '':
                                        ...
                                        if self.method == 'create':
                                            print("attr_ids_list", attr_ids_list, attr_value_ids_list)
                                            if attr_value_ids_list and attr_ids_list:
                                                ...
                                                while i < len(attr_ids_list):
                                                    ...
                                                    i += 1
                                            created_product_tmpl._create_variant_ids()
                                            print("created_product_tmpl", created_product_tmpl,
                                                  created_product_tmpl.product_variant_ids)
                                            if created_product_tmpl.product_variant_ids:
                                                product_var_obj = self.env['product.product']
                                                domain = [('product_tmpl_id', '=', created_product_tmpl.id)]
                                                ...
                                                if attr_value_price_dic:
                                                    ...
                                                    for product_varient_attribute_value_id in product_varient.product_template_attribute_value_ids:
                                                        if attr_value_price_dic.get(
                                                                product_varient_attribute_value_id.name, False):
                                                            extra_price = 0
                                                            if attr_value_price_dic.get(
