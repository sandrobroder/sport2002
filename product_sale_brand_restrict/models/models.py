    ...
    def _cart_update(self, product_id=None, line_id=None, add_qty=0, set_qty=0, **kwargs):
        if request.website and request.website.enable_brand_restriction:
            if product_id:
                ...
                if product.product_brand_id and partner_id.restricted_brand_ids:
                    if product.product_brand_id.id in partner_id.restricted_brand_ids.ids:
                        raise UserError(
                            "Sorry, You are not allowed to buy the products of %s brand!" % product.product_brand_id.name)
